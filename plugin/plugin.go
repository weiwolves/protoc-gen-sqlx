...
func (p *SqlxPlugin) generateTableNameFunction(message *generator.Descriptor) {
	...
	p.P(`func `, typeName, `TableName() string {`)
...
func (p *SqlxPlugin) generateMBboxStructure(message *generator.Descriptor, sqlDriver string) {
	...
	p.P(`p := Query`, typeName, `{
		Verbose: verbose,
		fields:   `, typeName, `Fields(),
		defFields: `, typeName, `FieldsMap(),
		limit:   1000,
		offset:  0,
		sort:    "id",
		order:   "ASC",`)
	p.P(`}`)
	p.P(`p.table = `, typeName, `TableName()`)
	p.P(`p.field = `, typeName, `FieldsString()`)
	p.P(`return &p`)
	p.P(`}`)
	p.P()
...
func (p *SqlxPlugin) generateMBboxFields(message *generator.Descriptor) {
	...
	if len(fields) > 0 {
		p.P(`func `, typeName, `FieldsString() string  {`)
		p.P(`return "`, fields, `"`)
		p.P(`}`)
		p.P()
		...
		p.P(`func `, typeName, `FieldsMap() (map[string]interface{}) {`)
